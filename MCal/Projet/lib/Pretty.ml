...
let (set_output_format: format -> unit) = fun format ->
      let output = 
	match format with
	| Latex -> _LATEX
	| Html  -> _HTML
...
let (extension: output -> string) = fun output ->
...
let (get_format: unit -> format) = fun () -> !_output.format ;;

let (get_format_ext: unit -> string) = fun () ->
      let ext = extension (!_output) in
...
let (angle: string -> string) = fun str ->
      let (l,r) =
	match !_output.format with 
	| Latex -> Latex.symbol "langle" , Latex.symbol "rangle"
	| _ -> "<",">"
      in l ^ str ^ r

let (brace: string -> string) = fun str ->
      let (l,r) = 
	match !_output.format with 
...
let (bracket: string -> string) = fun str ->
      let (l,r) = 
	match !_output.format with 
...
let (bold: string -> string) = fun string ->
  match !_output.format with 
...
let (italic: string -> string) = fun string ->
  match !_output.format with 
...
let (subscript: string -> string) = fun string ->
  match !_output.format with
...
let (supscript: string -> string) = fun string ->
  match !_output.format with
...
let (math: string -> string) = fun string ->
  match !_output.format with
...
let (blockquote: string -> string) = fun string ->
  match !_output.format with
...
let (enumerate: string list -> string) = fun strings ->
  match !_output.format with
  | Html -> 
	  String.concat "" 
	    [ "<OL>\n" 
...
let (select_color: int -> string * lumen) = fun i -> 
...
let (text_color: int -> string -> string) = fun i text ->
      match !_output.format with
...
let (background_color: int -> string -> string) = fun i text ->
      match !_output.format with
...
let (* OLD *) (color: string -> string -> string) = fun color_name text ->
      match !_output.format with
