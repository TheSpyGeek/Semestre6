...
(* LOGGER as object to allow multiple logs in different files
 *  
     let logger1 = new log_in "f1.log" in trace logger1 (fun _ -> f 1)
     let logger2 = new log_in "f2.log" in trace logger2 (fun _ -> f 2)
 *)
	
class logger = fun (opt_name: string option) ->
      object(self)

	val mutable opt_filename: string option = 
	  match opt_name with
	  | None      -> None
	  | Some name -> Some (String.concat "_" [ "log" ; Date.pretty_time() ; name ])

	val channel: out_channel =
	  match opt_name with
	  | None      -> stdout
	  | Some name -> open_out (String.concat "_" [ "log" ; Date.pretty_time() ; name ])
...
class logger_too_complicated = fun (name:string) ->
	...
	method log_in: string -> unit = fun name ->
	      begin
		logname <- String.concat "" [ name ; "_log_" ; Date.pretty_time() ; Pretty.get_format_ext() ] ;
     		self#log_on
	      end
