...
module Lambda_Term =
  (struct
    ...
    let (example3: t) = Apply[unit; s; z] 
    let (example4: t) = Apply[ Apply[incr;zero]; s; z] 
    let (example5: t) = Apply[ Apply[sum;unit;zero]; s; z] 
    let (example6: t) = Lambda(x, Apply[unit; s; z])  
    let (example7: t) = Apply[ Lambda(x, Apply[unit; s; z]) ; diverge ]     
  ...
  (struct
   ...
   (* TRACING *)	  
    ...
    let (trace_call: string -> t -> t delayed_computation -> t) = fun function_name t delayed_computation ->
	  Trace.call
	    [ function_name ; Lambda_Term.pretty t ]
	    Lambda_Term.pretty
	    delayed_computation
    ...
    let (trace_beta: (t * t) -> t -> t delayed_computation -> t) = fun (var,arg) expr delayed_computation ->
	  Trace.call
	    [ "*"
	    ; Lambda_Term.pretty (Apply[ Lambda(var,expr) ; arg])
	    ; "  by beta_reduction"
	    ; "[" ; Lambda_Term.pretty var ; "/" ; Lambda_Term.pretty arg ; "]"
	    ]
	    Lambda_Term.pretty
	    delayed_computation
   ...
   (* REDUCTION OF LAMBDA TERM BY BETA REDUCTION *)
    ...
    let (demo: unit -> t list) = fun () ->
	  [ i_reduce (Apply[zero; fS; fZ]) ;
	    i_reduce (Apply[unit; fS; fZ]) ; 
	    i_reduce (Apply[ Apply[incr;zero]; fS; fZ]) ;
	    i_reduce (Apply[ Apply[sum;unit;zero]; fS; fZ]) ;
	    i_reduce (Lambda(x, Apply[unit; fS; fZ])) ;
	    i_reduce (Apply[ Lambda(x, Apply[unit; fS; fZ]) ; diverge])     
	  ]
